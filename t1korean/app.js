...
function renderRecent(){
 const recent=$('#records');recent.replaceChildren();recent.classList.toggle('has-records',Boolean(records.length));
 $('#recent-controls').hidden=!records.length;
 if(!records.length){const b=document.createElement('strong');b.textContent=loaded?'첫 번째 승인된 기록을 기다리고 있어요.':'아카이브 연결을 준비하고 있어요.';const p=document.createElement('p');p.textContent='제보는 관리자 승인 후 사진과 함께 공개됩니다.';recent.append(b,p);return}
 records.slice().sort((a,b)=>(b.createdAt?.seconds??0)-(a.createdAt?.seconds??0)).slice(0,30).forEach(r=>{
  const card=document.createElement('article');card.className='record';
  const open=()=>{selected=players.findIndex(p=>p[0]===r.player);page=Math.floor((r.serial-1)/225);render();detail(r.serial)};
  if(r.photoPaths?.length){const photo=photoPreview(r.photoPaths[0],r.player+' #'+pad(r.serial)+' 제보 사진',{compact:true});card.append(photo)}
  else {const empty=document.createElement('div');empty.className='photo-placeholder';empty.textContent='출처 링크로 제보된 카드';card.append(empty)}
  const b=document.createElement('button');b.className='record-open';b.textContent=r.player+' · #'+pad(r.serial)+' ↗';b.onclick=open;
  const caption=document.createElement('small');caption.textContent=r.nickname||'익명 제보';card.append(b,caption);
  card.querySelector('img')?.addEventListener('click',open);recent.append(card);
 });
}
function detail(n){
 activeSerial=n;$('#detail-title').textContent=players[selected][0]+' #'+pad(n);
 const area=$('#detail-body');area.replaceChildren();const record=records.find(r=>r.player===players[selected][0]&&r.serial===n);
 const p=document.createElement('p');p.textContent=record?'승인된 개봉 기록 · 한국판':'아직 공개된 제보가 없습니다. 승인 대기 중이거나 미확인인 번호입니다.';area.append(p);
 if(record){
  for(const path of record.photoPaths??[])area.append(photoPreview(path,record.player+' '+record.serial+'번 제보 사진'));
